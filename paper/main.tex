...
data as a generalization of partial correlation by combining a mixed data
residualization approach with canonical correlations. Assuming that we need to
compute the association, $ \rho(X, Y; Z) $ between variables $ X $ and $ Y $
conditioned on $ \bm{Z} $. Similar to partial correlations, we first start by
computing the residuals for both $ X $ and $ Y $ using $ \bm{Z} $ as the
predictor variables. When $ \bm{Z} = \emptyset $ we can use a constant value
for $ \bm{Z} $. We use the mixed data residualization approach presented in
Ankan and Textor 2023.
