...
\begin{abstract}
	...
	studied for both DAGs and SEMs. For DAGs, numerous automated algorithms
	have been developed that can automatically estimate the structure from
	a given dataset. However, the adaption of these algorithms in applied
	...
	manually based on domain expertise which makes it vital to test the fit
	of these models to the data and potentially modifying them to improve
	the fit. On the other hand, in the field of SEMs, models are
	...
	is commonly known as \emph{specification search} where expert knowledge
	combined with methods like \emph{modification indices} can be used to 
	guide the modification process. In the case of DAGs, although
	statistical tests are available that can assess the global fit of
	model, but there are no methods that can guide our modification
	process. In this paper, we present a simple modification process aimed
	at helping researchers in manually constructing or modifying the DAGs
	outputted by an algorithm to improve its fit. This modification process
	identifies pairs of variables whose association is not fully explained
	by the model. Once such an unexplained association is found, an expert
	can start by adding edges between the worst offending pair of variables 
	while using their expertise to decide the direction between
	the variables. For the measure of association, in the case of
	continuous and discrete variables we can use effect sizes, however no
	such measure of association is present for mixed data. We propose a
	novel (conditional) measure of association for mixed data based on
	canonical correlations. We also present a graphical web-tool that can
	assist researchers in iteratively modifying and constructing their
	DAGs. We theoretically show that in the presence of an oracle, this
	iterative modification process is consistent in recovering the true
	DAG. Emprically, we show that this process performs comparably to PC
	and Hill-Climb Search algorithms if the user is able to correctly
