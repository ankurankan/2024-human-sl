...
mixed data by extending partial correlation to mixed data. We do this by
combining a mixed-data residualization method with a multivariate measure of
association. Given two variables $ X $ and $ Y $, and a set of conditional
variables $ \bm{Z} $, we are interested in computing an estimate of the
conditional association, $ \rho(X, Y; Z) $. Firstly, we compute the residuals
for $ x $ and $ y $ using the $ \bm{z} $ as the predictor variables. We use the
mixed data residualization approach from \citet{Ankan2023}. We consider each
variable type separately and repeat the following steps for both variables $ X
$ and $ Y $:
