...
\begin{abstract}
	...
	remain limited, possibly due to a lack of confidence in these
	...
	expert knowledge combined with methods like \emph{modification indices}
	is used to guide the modification process. In the case of DAGs,
	although statistical tests are available that can assess the fit of the
	model, but there are no methods that can guide our modification
	process. 

	In this paper, we present a method to guide the modification process aimed
	...
	identifies pairs of variables whose association in data is not fully explained
	by the model. Once such an unexplained association is found, an expert
	can iteratively add edges between the worst offending pair of variables 
	...
	DAG. Empirically, we show that this process performs comparably to PC
