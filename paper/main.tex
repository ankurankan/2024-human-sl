...
potential edge at random based on the iteration order in the algorithm.
However, if the residual association between the selected variables is low,
adding the edge may result in only a marginal improvement in the model's
overall fit. As the algorithm requires manual intervention in each iteration in
terms of domain knowledge, it would be beneficial to prioritize the edges that
improve the model's fit the most.

Therefore, an alternative to selecting an edge at random is to rank potential
edges based on the residual association between the variables given the DAG.
For quantifying the residual association between variables, we can use the
effect size measures of the CI test. So, for a d-separation query $ D_G(X, Y,
\mathbf{Z}) $, we can quantify the residual association between $ X $ and $ Y $
conditioned on $ \mathbf{Z} $, $ \phi(X, Y, \bm{Z}) $ as the effect size of the
CI test $ X \ci Y | \mathbf{Z} $. A greedy approach could be to choose the edge
that has the highest absolute residual association between the variables.
Algorithm~\ref{algo:ranked_expand} outlines this approach. This approach has
the benefit that it prioritizes edges that improve the model fit the most. We use
the \textsc{RankedExpand} in all of our experiments in the next section.
...
\begin{figure}[t!]
	...
	for the edge orientations. (a) Shows using total residual association that is the
...
\begin{equation}
	Y = \bm{\beta} \mathrm{Pa}_G(Y) + \mathcal{N}(0, 1)
\end{equation}
...
between the pair of variables with the highest residual association. While
...
Given a DAG $ G = (V, E) $, the total residual association, $\tau$ in define as:
\begin{equation}
...
p-value threshold and a measure of association threshold. To make the method
