...
\begin{abstract}
	...
	usuallly only get us to markov equivance class where the remaining
	edges need to be somehow oriented before inference can be done from
	them. Consequently, in applied disciplines, these Directed Acyclic
	Graphs (DAGs) are predominantly constructed manually based on domain
	expertise. Due to being constructed by hand, testing these models is
	vital before making any inference using them. Local testing in these
	models can be done by checking whether the implied conditional
	independences (CIs) by the model hold in the data or not. (Shipley
	2000) also proposed a global test based on combining these local tests.
	Based on these local tests, in the case of continuous or discrete
	variables, we can start by modifying the worst offending test.
\end{abstract}
