...
relationships among variables in a dataset.  To perform any causal effect
estimation using Directed Acyclic Graphs (DAGs) or Structural Equation Models
(SEMs), researchers need to first construct a causal structure. In the field of
DAGs, a lot of attention has been put on developing automated algorithms that
can construct these DAGs from data. These algorithm takes different approaches
to causal discovery such as constraint-based algorithms such as PC and Fast
Causal Infenrece (FCI) where the algorithm tries to match the conditional
...
optimization problem such as No Tears, and some hybrid approaches.
