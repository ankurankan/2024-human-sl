...
is called a \emph{$G$-compatible ancestral oracle}. If $H$ is furthermore acyclic, then 
...
\Fn{{\sc Expand}($V,E,\mathcal{D}_G,\mathcal{A}_G$,$B$,$k$)}{
	...
	$\{X,Y\} \notin B$}{
	    ...
	    \If{ $ (V, E \cup L) $ is cyclic }{
		    $ L' \gets \textsc{FixCycles}(V, E \cup L, \mathcal{D}_G) $ \;
		    $ B \gets B \cup (L \setminus L') $ \;
		    $ L \gets L'$
	    }
...
\SetKw{KwGoTo}{go to}
\SetKwProg{Fn}{Function}{:}{}
\Fn{{\sc FixCycles}($V, E, \mathcal{D}_G $)}{
		$R \gets \emptyset$ \; 
		\ForEach {$ X \rightarrow Y $ on a cycle in $ (V,E) $}{
		\If {there exists a set $\mathbf{Z}\subseteq\mathbf{X}$ where $\mathcal{D}_G(X, Y, \mathbf{Z})=1$
}{
			$ R \gets R \cup \{ X \rightarrow Y \} $ \;
...
When using experts that generate cyclic graphs, edges are pruned to 
break these cycles, which can lead to a more sparse result.

\begin{proposition}
For a conditional independence oracle
 $\mathcal{D}_G$ and a $G$-compatible ancestral oracle $\mathcal{A}_H$ where
$H$ contains at least one cycle,
let $\tilde{G}=\textsc{Expand}(V,\emptyset,\mathcal{D}_G,\mathcal{A}_H, \emptyset, \infty)$
Then $H^+ \supsetneq \tilde{G} \supseteq G$. 
...
cycle contains at least one edge that is not the skeleton of $G$, otherwise $G$ itself
would be cyclic. At least one edge of every cycle is therefore removed by
 $\textsc{FixCycles}$, making the result acyclic again.
\end{proof}
...
By combining Propositions~\ref{prop:strongexpand}, \ref{prop:weakexpand}, 
...
let $(V',E',B)=\textsc{Expand}((V,\emptyset),\mathcal{D}_G,\mathcal{A}_H,\emptyset,\infty)$.
...
\Repeat{ $E=E_p$ }{
	$E \gets E_p$ \;
	$(V,E,B) \gets \textsc{Expand}(V,E,\mathcal{D}_G,\mathcal{A}_G,B,1)$ \;
