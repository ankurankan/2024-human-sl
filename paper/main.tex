...
\begin{abstract}
	...
	for a given dataset. Even if using these algorithms, researchers have
	to rely on their expert knowledge to make modifications to the learned
	model in terms of fixing any mistakes or orienting the edges before
	these models can be used for any further analysis. Given this reliance
	on manual model modification, in this paper, we present a method
	inspired by \emph{specification search} in \emph{Structural Equation
	Models} (SEMs) to assist researchers in iteratively modifying or
	constructing their models to improve their fit to data. Specifically,
	we utilize implied Conditional Independence (CI) based model testing
	combined with a novel measure of partial association for mixed data to
	rank violations of implied CIs of the model in data. Based on this
	ranking, users can then choose to make modifications to the model by
	either removing or adding edges while using their expertise to decide
	the orientation of the edge. The ranking allows users to prioritize the
	worst offending violations first. Through empirical analysis, when
	applied to constructing the model from scratch, we demonstrate that
	taking a greedy approach (i.e., fixing the worst violation first) gets
	us comparable performance to automated algorithms if the user is able
	to correctly provide the edge direction in $ 1 $ out of $ 3 $ prompts.
	We further show that in the absence of an expert, a Large Language
	Model (LLM) can also be used for getting the edge orientations.
\end{abstract}
