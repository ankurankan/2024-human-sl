...
For $X, Y$ that are only $d$-connected through common ancestors, 
...
\begin{figure}[t!]
	...
	\begin{subfigure}{0.125 \textwidth}
		\centering
		\includegraphics[page=8]{example_cycle.pdf}
		\caption{$ X_3 \rightarrow X_4 $. $ X_1 \not \rightarrow X_4 $.}
	\end{subfigure}

	\caption{An example showing each iteration of \textsc{ExpertInLoop} when a cycle gets formed. The dashed edges show all the potential edges that \textsc{Expand} could add in each iteration.}
\end{figure}
