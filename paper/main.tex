...
\begin{abstract}
	Causal Discovery has received substantial attention in the DAG
	literature and many automated algorithms have been proposed for
	constructing DAGs from data. However, the application of these
	algorithms in applied fields remain limited due to challenges in using
	them in practice for example they make obvious mistakes, output CPDAGs
	that need to be oriented using expert knowledge before estimation
	methods can be applied. As a result of these challenges, researchers
	typically construct these model by hand. In this paper, inspired by
	specification search in SEMs where the method suggests changes in the
	model that improves the fit the most, we present a method based on
	Conditional Independence testing and a novel measure of partial
	association. The method iteratively suggests addition and removal of
	edges between pairs of variables, while the user can choose the
	direction of the edge. Using empirical analysis we show that this
	manual construction approach can outperform automated algorithms if the
	user gets the direction correct in <x out of y> cases. We also show
	that the user expert can be replaced with an LLM.
\end{abstract}
