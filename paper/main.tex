...
between $ X $ and $ Y $ using $ \Sigma_{XY} $. In this paper, we
...
causal relationship between the variables. The set of \emph{parents} of $ X $ in $ G $
are denoted as $ \textrm{Pa}_G(X) $, and its \emph{ancestors} and \emph{descendants} as $
\textrm{An}_G(X) $ and $ \textrm{De}_G(X) $ respectively. We denote a \emph{path}, $
\pi(X, Y) = \{ X, V_0, V_1, \cdots, V_k, Y \} $ between $ X $ and $ Y $ in $ G
$ where each consecutive pair of variables in $ \pi $ are connected through an
edge. We say $ X $ and $ Y $ are \emph{d-connected} in $ G $ given a conditioning set
...
In this section, we show an example of how this model testing works in
...
\emph{Relationship}. We can then test each of the implied CIs of the DAG
against our dataset using a statistical test. Figure~\ref{fig:ci_table} shows
some of the implied CIs of the model and the corresponding p-value of
statistical test. Using a p-value threshold of $ 0.05 $, we can determine that
the first $2$ implied CIs hold in the data while the remaining two do not.
These failing tests tell us that there is residual association between these
variable pairs that is currently not being explained by the DAG. This method,
however, does not give us any information on the reason, such as there is a
direct or indirect effect, or a confounding variable, behind this observed
association. To fix these failing tests, we still have to rely on domain
knowledge to determine the type of relationship and the causal directions
