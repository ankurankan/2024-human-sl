...
model construction process is commonly known as \emph{Specification Search}
\citep{Long1983} and uses method like modification indices, and Wald-based
...
\begin{enumerate}
	\item \textbf{Lack of Trust:} Although most algorithms are
		...
		significantly depending on the algorithm and their
...
Figure~\ref{fig:intro} shows some of these issues when applied to the Adult
Income dataset. Due to these challenges, whether constructing the model
manually or using an algorithm, we typically have to rely on some expert
knowledge to get a model that we can use for further analysis. Hence, it is
crucial to have tools to guide this construction or modification process.
...
\begin{enumerate}
	...
	\item Lastly, we present both a web-browser based tool and an
...
In this section, we develop a novel measure of partial association for mixed
data by extending partial correlations \citep{Whittaker2009} \todo{Verify this
citation} to mixed data. To compute partial correlations we use: 1) A
residualization method, and 2) a measure of associations between the residuals.
In the case when both $ X $ and $ Y $ are continuous variables, the residuals
are typically computed using a regression based method and  Pearson's
correlation coefficient is used as a measure of association between the
residuals. To extend this idea to mixed data, we combining a mixed data
residualization method \citep{Ankan2023} with a multivariate measure of
association based, Pillai's Trace \citep{Pillai1955}. 

Given $ n $ samples $ D = (x, y, \bm{z}) $ of variables $ X $, $ Y $, and $
\bm{Z} $, we are interested in computing an estimate of the conditional
association $ \rho(X, Y; \bm{Z}) $ between $ X $ and $ Y $ conditioned on $
\bm{Z} $. To compute the residuals, we utilize the residualization approach
from \citet{Ankan2023}. Depending on the type of dependent variable, the
...
\begin{enumerate}
	\item \textbf{Bounded: } 
		...
		between $ 0 $ and $ 1 $. This allows for interpretability and standardization to allow us to compare.
...
\todo[inline]{Show an example where local changes can get stuck}
