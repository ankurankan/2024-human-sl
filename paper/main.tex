...
In Algorithm~\ref{algo:expert}, in each step we add a single edge to the DAG
and remove any edges that can be pruned. In practice, we use a slightly
modified version of the Algorithm~\ref{algo:expand} for adding new edges as
given in Algorithm~\ref{algo:ranked_expand}. Rather than choosing the first
potential edge addition, this algorithm ranks all potential new edges based on
the conditional association between the variables. The edge with the highest
conditional association is then added to the graph. This modification allows us
to prioritize edges that would help in the explaining the observed correlation
in the data the highest. For this purpose, we use a measure of conditional
association measure, $\phi$ between variables.
