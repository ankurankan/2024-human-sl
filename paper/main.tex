...
simulate an imperfect domain expert, we use a version of the strong ancestral oracle
	...
	\begin{split}
		...
		\mathrm{Expert}(\alpha) &= \begin{cases} 
			...
			\mathcal{O}(X, Y) &= \begin{cases}
				X \rightarrow Y, & \textrm{if } X \in \textrm{An}_{G_{\textrm{true}}}(Y) \\
				X \leftarrow Y, & \textrm{if } Y \in \textrm{An}_{G_{\textrm{true}}}(X) \\
				\textrm{None}, & \textrm{otherwise }
				  \end{cases}
	\end{split}
\end{equation}
...
higher than $ \alpha $ as even when $ x > \alpha $, there is a $ 1 $ in
$ 3 $ chance that edge orientation is correct. Therefore, an $ \mathrm{Expert} $ with
...
$\alpha=0.05$; note that this means that also the d-separation ``oracle''
makes both type~I and type~II errors. For the analysis, we
simulate data from known DAGs and use the algorithms to recover the original
...
$$
where each coefficient $\beta_i$ of the vector $\bm{\beta} \in \mathbb{R}^{|\bm{\beta} |}$ is randomly drawn as
$$
...
of a temporal structure between the variables \cite{Bang2023}.
