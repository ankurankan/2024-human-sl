...
\Fn{{\sc FixCycles}($V, E, \mathcal{D}_G $)}{
		$R \gets \emptyset$ \; 
		\ForEach {$ x_i \rightarrow x_j $ on a cycle in $ (V,E) $}{
		\If {there exists a set $\mathbf{Z}\subseteq\mathbf{X}$ where $\mathcal{D}_G(x_i, x_j)=1$
}{
			$ R \gets R \cup \{ x_i \rightarrow x_j \} $ \;
...
$B \gets \emptyset$ \tcc{Edges that were pruned or removed from cycles} 
\Repeat{ $E=E_p$ }{
	$E \gets E_p$ \;
	$(V,E,B<D-s>) \gets \textsc{Expand}(V,E,\mathcal{D}_G,\mathcal{A}_G,B,1)$ \;
