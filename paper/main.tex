...
DAG, enabling users to prioritize modifications while utilizing their expertise
when making these modifications. Empirical results show that this manual model
construction approach achieves performance comparable to automated algorithms
even if the user is able to decide the orientation of an edge correctly in one
out of three cases. Additionally, we demonstrate that, in the absence of an
expert, a Large Language Model (LLM) can potentially determine edge orientations.
...
\begin{enumerate}
	\item \textbf{Lack of Trust:} While most algorithms are asymptotically
		...
		robust performance evaluation methods further reduce the
		confidence in their outputs. As a result, even when using these
		algorithms are used, researchers often rely on post hoc manual
