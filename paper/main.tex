...
and Structural Equation Models (SEMs) literatures, each adopting different
approaches. In the DAG literature, the focus has primarily been on developing
automated algorithms that can learn the causal structure from observational
...
As a result of these efforts, numerous approaches to automated algorithms for
causal discovery have been proposed in the DAG literature, including
constraint-based method like the PC algorithm \citep{Spirtes2001} and Fast
...
DAGMA \citep{Bello2022}. However, the adaption of these algorithms in applied
fields remain limited, and researchers typically prefer to construct DAGs
manually based on their domain expertise \citep{Tennant2020, Petersen2021}.
Even when using automated algorithms, users often rely on manual post hoc
modifications to the model learned by the algorithm because they:
...
\begin{enumerate}
	\item \textbf{Make Obvious Mistakes:} Although most algorithms are
		...
		real-world settings with finite samples, these algorithms can
		make obvious mistakes. Consequently, users have to rely on
		posthoc modifications to the model based on their domain
		expertise.
	\item \textbf{Outputs Markov Equivalence Class (MEC):} Multiple
		...
		a fully oriented DAG. As a result, users have to manually
...
algorithm, users must rely on their domain expertise to construct or modify the
...
\end{figure}
\begin{figure}
	\caption{\todo[inline]{Figure showing an example of modification indices}}
\end{figure}
...
knowledge decide the direction of the edge. Our main contributions in this
...
\begin{enumerate}
	...
	\item Using this partial association measure, we present a procedure to
		rank CI tests. This ranking can in turn then be used to
		prioritize addition or deletion of edges from the model.
...
well. For linear datasets users can also choose variants of linear regression
...
CI tests. If the user desires to construct the model manually, they can also
start with an empty graph, or start with a DAG outputted by a causal discovery
algorithm. Given a DAG $ G $ and a dataset $ D $, for each pair for variable
$ X $ and $ Y $ in $ G $ that does not have an edge between them, we compute
the partial association measure: $ \rho(X, Y; pa_G(X) \cup pa_G(Y)) $. The
measure of association in this case can be interpreted as the unexplained
...
\begin{figure}
	...
	multiple accuracy values of the oracle. \todo[inline]{Replace Hill-Climb with GES}}
\end{figure}

We analyzed the performance of this manual approach by comparing it with two
automated algorithms: PC and Greedy Equivalence Search. For the comparison we
use simulated data from a known DAG and compare how well the algorithm is able
to recover the original model using two metrics: Structural Hamming Distance
(SHD) and Structural Intervention Distance (SID). To simulate the data, we
start with a randomly generated DAG and use linear models with random effects
to generate the data. We repeat the data generation $ 30 $ times for each
density value for the DAG. As both PC and Hill-Climb Search can only give us a
CPDAG, we compare the orientation of these CPDAG which result in the best and
worst case scores on SHD and SID. This procedure is outlined in Algorithm 1.
best and worst case scores on SHD and SID. This procedure is outlined in
Algorithm 1.
