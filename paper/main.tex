...
In this section, we develop a novel partial measure of association for
...
the conditional association, $ \rho(X, Y; \bm{Z}) $. If $ \bm{Z} = \emptyset $, we
use a constant value as $ Z $. Firstly, we compute the residuals for $ \bm{x} $ and
...
association measure: $ \rho(X, Y; pa_G(X) \cup pa_G(Y)) $. Additionally, for
each pair of variable in $ G $, that has an edge between them in we compute the
partial association measure: $ \rho(X, Y; pa_{\underline{G}}(X) \cup
pa_{\underline{G}}(Y) $. In the case of no edge, this measure of association
can be interpreted as the unexplained 
...
\begin{theorem}
If we have access to an oracle that can give correct edge directions, the
algorithm above is consistent.
\end{theorem}
