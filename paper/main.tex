...
\begin{enumerate}
	\item \textbf{Bounded: } $ \rho $ is bounded between $ 0 $ and $ 1 $,
		with $ 0 $ representing no association while $ 1 $ representing
	...
	\item \textbf{Invariant to the dummy encoding scheme: } As we use dummy
		encoding to compute the residuals for categorical variables,
		but as canonical correlations search for linear combinations
		over the columns, the measure is invariant to the dummy
		encoding scheme that we use.
	\item \textbf{Generalizes partial correlations to mixed data: }
		As canonical correlation is equivalent to Pearson's correlation
		coefficient, when both $ X $ and $ Y $ are continuous, $ \rho $
		is equivalent to canonical correlations. Hence, $ \rho $ is a
		generalization of partial correlations to mixed data.
