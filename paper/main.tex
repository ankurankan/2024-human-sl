...
In this paper, we propose a method that leverages this CI testing approach to
assist researchers during the DAG construction process, rather than just
validating the model. Our approach follows an iterative model-building strategy
that integrates domain expertise with CI testing on data. At each iterations,
we use CI testing to identify pairs of variables where an edge is either
missing or superfluous. These pairs are then presented to the expert, who
determines whether to add or remove these edges. The orientation of the edge
when adding is determined based on the domain expertise. As at any iteration,
multiple changes to the DAG are possible. We also give a way to rank potential
new edges additions to the DAG using effect size measures for CI tests to
address the most severe violations of CI tests first. As a result this approach
acts like a guide to the expert when manually constructing their model while
ensuring that the model is consistent with the data. 
