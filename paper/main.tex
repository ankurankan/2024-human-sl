...
\title{Expert-In-The-Loop Causal Discovery: An Iterative Approach for Efficiently Using Expert Knowledge in Model Construction}
\author{
    %Authors
...
\begin{abstract}
	...
	numerous automated algorithms for constructing DAGs from data. Despite
	these advancements, the application of these algorithms in applied
	domains remain limited, with researchers often preferring to construct
	DAGs manually based on their expert knowledge. We hypothesize that the
	reluctance to use automated algorithms arise due to issues with these
	algorithms such as their tendency to make obvious mistakes, and
	outputting the Markov Equivalence Class (MEC) instead of a DAG, which
	is often required for further analysis. Even when using these
	algorithms, researchers have to rely on their expert knowledge to make
	modifications to the learned model. Given this reliance on manual model
	construction and modification, in this paper, we introduce a method to
	assist researchers in this task inspired by specification search in
	Structural Equation Models(SEMs). We present an iterative approach to
	model modification utilizing Conditional Independence (CI) testing
	combined with a novel measure of partial association for mixed data to
	rank potential additions or removal of edges to the DAG such that the
	improvement in the model's explanation of the data is improved the
	most. Specifically, the method ranks violations of implied conditional
	independences (CI) in the data using the measure of partial
	association. The user can choose to fix these violations by adding
	edges between the pair of variables while using their domain expertise
	to decide the direction of edge between the variables. Through
	empirical analysis, we demonstrate that taking a greedy approach (i.e.
	fixing the worst ranked violation first) with this manual construction
	approach can get comparable performance to automated algorithms if the
	user are able to get the edge direction correctly between the variables
	at least $ 1 $ out of $ 3 $ prompts (check if this is correct).
	Alternatively, we show that a Large Language Model (LLM) can also be
	used to help in deciding the direction between the variables.
\end{abstract}
