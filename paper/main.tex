...
address these challenges, we propose an iterative DAG construction method that
combines domain knowledge with data-driven insights. Our approach iteratively
identifies pair of variables with a missing edge in the DAG, and asks for the
orientation of the edge from an expert. We also propose a method to rank these
potential new edges based on their impact on the model fit. Empirical results
show that this iterative model construction approach achieves performance
comparable to automated algorithms if we are able to correctly decide the
orientation of new edges in one out of three cases. Additionally, we
demonstrate that, in the absence of an expert, a Large Language Model (LLM) can
be used to determine edge orientations. We provide the implementation of our
approach in both a web tool at: <redacted for review> and a Python package
<redacted for review>.
...
% decide the orientation of new edges. 
...
Figure~\ref{fig:intro} highlights some of these issues. To assist researchers
in constructing DAGs manually, we propose an iterative model construction
method that combines domain knowledge with data-driven insights. Our approach
utilizes conditional independence testing to identify pair of variables in the
DAG that are missing an edge or have a superfluous edge. Based on this
information, in each iteration we present one such pair of variables to an
expert who then decides the orientation of the edge between the variables based
on their domain knowledge. The method also iteratively prunes all superfluous
edges in each iteration. As a result this approach guides the expert in
