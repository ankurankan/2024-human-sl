...
relationships among variables in a dataset. Causal discovery techniques are
used typically to understand and estimate causal effects between variables. In
the To perform any causal effect estimation using Directed Acyclic Graphs
(DAGs) or Structural Equation Models (SEMs), researchers need to first
construct a causal structure. In the field of DAGs, a lot of attention has been
put on developing automated algorithms that can construct these DAGs from data.
These algorithm takes different approaches to causal discovery such as
...
problem is formulated as a constrained optimization problem such as No Tears, and
...
researchers in suggesting best ways to modify the model. For example,
\emph{modification indices} take the initial drawn model, and make all possible
modifications to the model to check which modification improves the model fit
the best. Based on this information, the researcher can make modifications to
the model and reiterate this process till they get something that they are
satisfied with.
