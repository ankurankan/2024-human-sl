...
\section{Background}
We consider one-dimensional variables $ X $, and $ Y $ and a possibly
multi-dimensional co$ Z = Z_1, \cdots, Z_k $, potentially being an empty set $
Z = \emptyset $. We consider various scenarios where the variables $ X $, $ Y $
and $ Z $ could be either discrete, ordinal, or continuous. With mixed data, we
mean an arbitrary mix of variable types among $ X $, $ Y $, and $ Z $.
...
\subsubsection{$ X $ and $ Y $ are continuous}
...
\subsubsection{All $ X $, $ Y $, $ Z $ are discrete}
...
\subsubsection{$ X $ is ordinal and $ Y $ is continuous}
...
\subsubsection{$ X $ and $ Y $ are ordinal}
...
Canonical correlations are generalization of correlations to multi-dimensional case.

Multiple effect sizes measures based on canonical correations are available.

\section{Effect size for mixed data}
In this section, we use canonical correlations to define an effect size for mixed data CI tests. The main idea is to transform any CI test into j
We consider the following two cases separately:

\subsection{No conditional variables}
When we have no conditonal variables, we are trying
...
\begin{figure}
	...
	\begin{subfigure}{0.5\textwidth}
		\includegraphics[scale=0.25]{../../presentations/2024_05_das/1.png}
	\end{subfigure}%
	\begin{subfigure}{0.5\textwidth}
		\includegraphics[scale=0.25]{../../presentations/2024_05_das/2.png}
	\end{subfigure}
	\caption{\todo[inline]{Insert screenshots of the web-tool}}
\end{figure}

\section{Empirical Analysis}
To compare how well does creating a graph manually based on effect sizes and
p-value compare to algorithmic approaches, we performed some emperical
analysis.
...
\todo[inline]{Summarize this section and equaiton in an algorithm}
