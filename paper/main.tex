...
$ X $ and $ Y $ that are part of a directed graph $G$, an \emph{strong ancestral oracle} 
...
such that $E_G \subseteq E_H$. Then $\mathcal{A}_H$
is called a \emph{$G$-consistent ancestral oracle}. Further, 
if $H=(V,E_H)$ is a supergraph of $G$ that contains at least one cycle, then 
$\mathcal{A}_H$ is called a \emph{$G$-consistent ancestral oracle} with respect to $G$.
\end{definition}
...
Further, the inconsistent ancestral oracle sometimes contradicts itself: there are at least two
variables $X,Y$ where its information implies both a causal effect of $X$ on $Y$ and a reverse 
causal effect of $Y$ on $X$. Therefore, if we use such oracles for DAG construction, we will
...
\Fn{{\sc Expand}($V,E,\mathcal{D}_G,\mathcal{A}_G$,$B$,$k$)}{
	...
	$\{X,Y\} \notin B$}{
	    ...
	    $L \gets L \cup \{ e \} $ \;
	    \If{ $ (V, E \cup L) $ is cyclic }{
		    $ L \gets \textsc{FixCycles}(V, E, L, e, \mathcal{D}_G) $ \;
	    }
	    % \If{ $(V,E \cup L \cup e)$ is acyclic }{
            % 	$L \gets L \cup  \{ e \} $\;
	    % } \Else {
            % 	$L \gets L \cup  \{ \textrm{reverse}(e) \} $\;
	    % }
        }
	\If{$|L|\geq k$}{\KwGoTo 16\;}
    }
...
\DontPrintSemicolon
\SetAlgoLined
\SetKwFunction{FixCycle}{FixCycle}
\SetKw{KwGoTo}{go to}
\SetKwProg{Fn}{Function}{:}{}
\Fn{{\sc FixCycles}($V, E, L, e, \mathcal{D}_G $)}{
	$ u, v \gets e $ \;
	\ForEach {$ P = \{ v \rightarrow x_1, x_1 \rightarrow x_2, \cdots,  x_{k-1} \rightarrow x_k, x_k \rightarrow u \} $ in ($V, E \cup L$) }{
		\ForEach {$ x_i \rightarrow x_j $ in $ P $}{
        	let $\mathbf{Z}$ be a set that $d$-separates $x_i$ and $x_j$ in $(V, E \cup L)$\;
		\If {$ \mathcal{D}_G(X, Y, \mathbf{Z}) = 0 $}{
			$ L \gets L - \{ x_i \rightarrow x_j \} $ \;
...
\begin{proposition}
For a conditional independence oracle
 $\mathcal{D}_G$ and a $G$-consistent ancestral oracle $\mathcal{A}_H$, 
