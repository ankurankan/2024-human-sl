...
In the case of continuous data, the residuals can be computed using a
regression based method and Pearson's correlation coefficient can be used for
computing the association between the residuals. We extend this idea to mixed
data by combining a mixed data residualization method \citep{Ankan2023} with a
multivariate measure of association based, Pillai's Trace \citep{Pillai1955}.
...
$ Z $. Similar to partial correlation, we first compute the residuals for $ X $
and $ Y $ using $ \bm{Z} $ as the predictor variables. We use the mixed data
residualization approach from \citet{Ankan2023}. Depending on the type of
dependent variable, the residuals are defined as follows:
