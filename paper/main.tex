...
To compare how well this manual appraoch assisted by the suggested modications
perform compared to automated learning algorithms, we performed an analysis
using simulated data and simulating an expert. To construct the model using
suggestions, we start with a model with no edges, and take a greedy approach
for adding edges. At any point, we select to add an edge between variables that
have the highest conditional association between them. A simulated expert is
then asked to specify the direction for the edge. Given an accuracy of the
expert $ \alpha $, the expert either gives the correct direction, incorrect
direction, or can specify do not know as:
...
If the expert says `do not know` for any of the edges, the edge is not prompted
again to the expert. We also make sure that only edges which do not form a
cyclic in the graph are used to select the next potential edge. After adding
each edge, we also check if the p-value of any existing edge shows
independence. If that happens we remove that edge and blacklist that edge,
i.e., that edge is not prompted again during the run of the algorithm. We
repeat this procedure till the model explains all correlations between the
variables. There is a possibility that this greedy expert gets stuck at
incorrect graph structures as it does not do any backtracking to fix its
earlier mistakes. This procedure is outlined in the Algorithm.
