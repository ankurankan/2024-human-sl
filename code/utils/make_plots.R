...
make_plots_v2 <- function(filename, plot_type){
	...
	d_shd <- d %>% select(edge_prob, oracle_acc, hc_lower_shd_mean, hc_upper_shd_mean, hc_lower_shd_sd, hc_upper_shd_sd, pc_lower_shd_mean, pc_upper_shd_mean, pc_lower_shd_sd, pc_upper_shd_sd, human_shd_mean, human_shd_sd)
	...
	d_shd <- d_shd %>% select(edge_prob, oracle_acc, hc_mean, hc_lower, hc_upper, pc_mean, pc_lower, pc_upper, human_mean, human_lower, human_upper)
	...
	d_shd <- as.data.frame(cbind(c(d_shd$edge_prob, d_shd$edge_prob, d_shd$edge_prob),
	      	       ...
	      	       c(d_shd$hc_mean, d_shd$pc_mean, d_shd$human_mean),
	      	       c(d_shd$hc_lower, d_shd$pc_lower, d_shd$human_lower),
	      	       c(d_shd$hc_upper, d_shd$pc_upper, d_shd$human_upper),
		       c(rep('Hill-Climb Search', nrow(d_shd)), rep('PC', nrow(d_shd)), rep('Expert', nrow(d_shd))),
		       c(rep('hc', nrow(d_shd)), rep('pc', nrow(d_shd)), paste('human', d_shd$oracle_acc)),
		       c(rep(NA, nrow(d_shd)), rep(NA, nrow(d_shd)), d_shd$oracle_acc)
		       ))
	...
	p_shd <- ggplot(d_shd, aes(x=edge_prob, y=mean_val, 
				   ...
				   fill=algo,
				   linetype=ifelse(algo %in% c('PC', 'Hill-Climb Search'), NA, "solid"))) + 
		geom_line(alpha=0.4, show.legend=T) +
		geom_point(data=subset(d_shd, !(algo %in% c('PC', 'Hill-Climb Search'))), pch=19, size=1, alpha=0.4, show.legend=F) +
	...
	ggsave("plots/shd_ribbon.pdf", p_shd, height=2.5, width=3.2, units='in')
	...
	ggsave("plots/sid_ribbon.pdf", p_sid, height=2.5, width=3.2, units='in')
}
