...
for (edge_prob in edge_probs){
	for (oracle_acc in oracle_accs){
		shd <- replicate(100, run.sim(n_nodes=n_nodes, edge_prob=edge_prob, oracle_acc=oracle_acc))
		mean_shd <- mean(shd)
		results <- c(results, mean_shd)
	}
}
run.sim(n_nodes=10, edge_prob=0.4, oracle_acc=0.1)
