...
\begin{figure}[H]
	\begin{subfigure}{0.5 \textwidth}
		\begin{tikzpicture}[scale=2]
				...
				\draw [->] (X) -- (x1) node[midway, above, sloped] {$0.3$};
				\draw [->] (X) -- (x2) node[midway, above, sloped] {$0.3$};
				\draw [dashed](X) to [bend left=15] (Y);
				\draw [dashed](x1) to [bend left=15] (x2);
				\draw [dashed](x1) to [bend right=25] (x3);
				\draw [dashed](x2) to [bend left=15] (x3);
				\draw [dashed](x3) -- (Y);
		\end{tikzpicture}
	\end{subfigure}%
	\begin{subfigure}{0.5 \textwidth}
		...
		\end{tikzpicture}
	\end{subfigure}
	\caption{(Left) Marginal (Right) Conditional remaining edges after true model has been discovered.}
\end{figure}
